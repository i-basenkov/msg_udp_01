...
namespace msg
{
	...
	namespace _detale
	{
		struct _block_thread_t
		{
			_block_thread_t() = default;
			_block_thread_t(_block_thread_t const&) = delete;
			_block_thread_t(_block_thread_t&&) = delete;
			_block_thread_t& operator=(_block_thread_t const&) = delete;
			_block_thread_t& operator=(_block_thread_t&&) = delete;

			template <typename _MV, typename _EV, typename _TO>
			constexpr
			auto& operator ()(thread_interface_t<_MV, _EV, blockable, _TO>& ti) const noexcept
			{
				{
					std::lock_guard<std::mutex> lg(ti.m_bmtx);
					ti.m_bv = blocked{};
				}
				ti.m_bcv.notify_one();
				return ti;
		...
		struct _unblock_thread_t
		{
			_unblock_thread_t() = default;
			_unblock_thread_t(_unblock_thread_t const&) = delete;
			_unblock_thread_t(_unblock_thread_t&&) = delete;
			_unblock_thread_t& operator=(_unblock_thread_t const&) = delete;
			_unblock_thread_t& operator=(_unblock_thread_t&&) = delete;
	...
	constexpr
	_detale::_block_thread_t block;
	constexpr
	_detale::_unblock_thread_t unblock;

	template <typename _MV, typename _EV, typename _TO>
	constexpr
	auto& operator | (thread_interface_t<_MV, _EV, blockable, _TO>& _ti_o
							, _detale::_block_thread_t const& _bt) noexcept
	{
		return _bt(_ti_o);
	}
	...
	constexpr
	auto& operator | (thread_interface_t<_MV, _EV, blockable, _TO>& _ti_o
							, _detale::_unblock_thread_t const& _bt) noexcept
	{
		return _bt(_ti_o);
	}
	...
	namespace _detale
	{
		...
		struct thread_worker_t
		<
			WT, _MV, _EV, blockable, _TO, _H, _CH
		>
			: __thread_worker_t<WT, _MV, _EV, blockable, _TO, _H, _CH>
		{
			using __thread_worker_t<WT, _MV, _EV, blockable, _TO, _H, _CH>::__thread_worker_t;
			...
			void operator()()  noexcept
			{
				...
				while (!this->thr_i.stop())
				{
					std::visit(overloaded_t
					{
						...
						[this, &cnt](blocked){
							{
								std::unique_lock<std::mutex> ul(this->thr_i.m_bmtx);
								if (this->thr_i.m_bcv.wait_for(ul, std::chrono::seconds(1))
